...
const getArticleBySlug = async (req, res) => {
   try {
      ...
      const newViewsCount = +views + 1 || 1;
      await Articles.update([{ pid: id, views: newViewsCount }]);
...
const editArticle = async (req, res) => {
   try {
      ...
      const article = await Articles.findOne({ pid: articleId })
      if (!article) {
